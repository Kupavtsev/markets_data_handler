...
def tr():
    ...
    for el in df.columns:
        ...
        result.append([el, df[el][number_of_session-1][0], tr])

    
    return result       # List of Lists (asset, timestamp, TR)
        

# I will start some cycle of deleting last element
...
def tr_test():
    ...
    for el in df.columns:
        print(el, '=>')
        # print(df[el])                       # All sessions by asset
        # print(len(df[el]))                  # number of sessions
        number_of_session = len(df[el])
        # print(df[el][number_of_session-1])            # session
        # print(df[el][number_of_session-1][0])         # timestamp os session
        price_day_high = float(df[el][number_of_session-1][2])
        price_day_low = float(df[el][number_of_session-1][3])
        price_day_close_prev = float(df[el][number_of_session-2][4])
        tr = max((price_day_high - price_day_low), (price_day_high - price_day_close_prev), (price_day_low - price_day_close_prev))
        ...
        result.append([el, df[el][number_of_session-1][0], tr])
        # counter -= 1
        # for session in df[el]:
        #     print(el, '=> ', session)
            # print(el, '=> ', session[0])         # timestamp os session
    return result